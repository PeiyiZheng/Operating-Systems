//1. Add definition of NSHM(16), NOSHM(4), NSHMPGS(16) into param.h.

//2. Add the definition of struct shmem into proc.h. Add shmem_table(array of shmem with a spin lock) into proc.c:
struct shmem {
  int ref_cnt;
  int owner;
  int not_published;
...
int shmmk(int id, int size) {
  ...
  if(shmem_table[id].ref_cnt > 0) {
    release(&shmem_table.lock);
    return -1;
  }

  shmem_table[id].ref_cnt = 1;
  size /= PGSIZE;
  int i;
  ...
  for(i = 0; i < size; ++i) {
    ...
    if(mem == 0) {
      ...
      --i;
      while(i >= 0) {
        kfree(shmem_table[id].shared_pages[i]);
        --i;
      }

      shmem_table[id].ref_cnt = 0;
      release(&shmem_table.lock);
      return -1;
    }

    shmem_table[id].shared_pages[i] = mem;
  }

  shmem_table[id].owner = getpid();
  shmem_table[id].region_size = size * PGSIZE;
  //convert the virtual address to the general physical address
  for (i = 0; i < size; i++){
    shmem_table[id].shared_pages[i] = V2P(shmem_table[id].shared_pages[i]);
  }
  // default value is 0 so that we don't need to initialize the shmem table
  shmem_table[id].not_published = 1;
  release(&shmem_table.lock);
  return 0;
}
...
int shmpub(int id) {
  ...
  if(!shmem_table[id].not_published) {
    release(&shmem_table.lock);
    return -1;
  }
  ...
  if(shmem_table[id].owner != pid) {
    release(&shmem_table.lock);
    return -1;
  }

  // published
  shmem_table[id].not_published = 0;
  release(&shmem_table.lock);
  return 0;
}
...
void *shmat(int id) {
  ...
  if(shmem_table[id].not_published && shmem_table[id].owner != getpid()) {
    release(&shmem_table.lock);
    return -1;
  }

  int i;
  if(shmem_table[id].map_proc_cnt > 0) {
    for(i = 0; i < shmem_table[id].map_proc_cnt; ++i) {
      // already mapped to this region
      if(shmem_table[id].map_proc[i] == getpid()) {
        release(&shmem_table.lock);
        return -1;
      }
    }
  ...
  for(i = 0; addr < shmem_table[id].region_size; addr += PGSIZE, ++i) {
    if(mappages(proc->pgdir, (char*)addr, PGSIZE, shmem_table[id].shared_pages[i], PTE_W|PTE_U) < 0){
      // if mappages fails, what should we do???
      // current solution: code from deallocuvm()
      pte_t *pte;
      uint pa;
      for(; start_addr < addr; start_addr += PGSIZE) {
        pte = walkpgdir(pgdir, (char*)start_addr, 0);
        if(!pte) {
  ...
  shmem_table[id].map_proc[shmem_table[id].map_proc_cnt] = getpid();
  ++shmem_table[id].map_proc_cnt;
  release(&shmem_table.lock);
  return start_addr;
}
...
int shmdt(void *shm) {
  ...
  int size = shmem_table[shmID].region_size;
  uint a = (uint)shm;
  uint pa;
  ...
  shmem_table[id].ref_cnt--;
  if (shmem_table[id].ref_cnt == 0) {
    //no procs pointing to that shared mem anymore
    //garbage collect it up. kfree every page of it
    int j;
    a = 0;
    for(j = 0; a < shmem_table[id].region_size; a += PGSIZE, ++j) {
      kfree(P2V(shmem_table[id].shared_pages[j]));
    }
  }
